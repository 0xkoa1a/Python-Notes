...
第二章已经从 OOP 设计角度讲过 `property` 的使用场景。这里关注更底层的问题：点号访问、赋值和删除会进入哪些协议入口，以及为什么大多数业务代码不应该直接重写这些入口。
...
很多校验场景用第二章讲过的 `property` 或本章后面的描述符更清楚。`__setattr__` 适合需要统一拦截多个属性的场景。
