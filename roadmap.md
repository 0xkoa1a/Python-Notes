...
正式笔记放在 `notes/`，可执行实验放在 `labs/`，阶段验收小项目放在 `projects/`，可复用代码片段放在 `snippets/`。
...
`notes/` 是长期沉淀，`labs/` 是运行时验证，`projects/` 是阶段性综合练习。三者使用同一套编号，方便互相映射。
...
2. 在 `labs/` 做最小实验，验证模型。
3. 把实验结论回写到笔记。
4. 在 `projects/` 做一个阶段小项目。
5. 回顾哪些现象曾经像 special case，现在能否被统一解释。
